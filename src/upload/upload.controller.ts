...
export class UploadController {
  ...
  @ApiOperation({
    ...
    description:
      'Upload an image for a post. Can be used for new posts or updating existing posts.',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Post image uploaded successfully.',
        },
        data: {
          type: 'object',
          properties: {
            imageUrl: {
              type: 'string',
              example:
                'https://your-project.supabase.co/storage/v1/object/public/posts/post-123/image.jpg',
            },
          },
        },
      },
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - invalid file or post ID',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - invalid or missing JWT token',
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - can only upload images for own posts',
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    description: 'Post image file and optional post ID',
    schema: {
      type: 'object',
      properties: {
        file: {
        ...
        postId: {
          type: 'string',
          description:
            'Post ID (optional - if not provided, image URL will be returned for later use)',
        },
      },
      required: ['file'],
    },
  ...
  async uploadPostImage(
    ...
    try {
      const authHeader = request.headers.authorization;
      const token = authHeader?.replace('Bearer ', '');
      const result = await this.uploadService.uploadPostImage(
        request.user.id,
        file,
        token,
        postId,
      ...
      throw new HttpException(
        {
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          message: 'An unexpected error occurred while uploading post image.',
          error: error.message,
        },
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
  ...
  @ApiOperation({
    ...
    description:
      'Update the image for an existing post. This will replace the current image with a new one.',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        ...
        message: {
          type: 'string',
          example: 'Post image updated successfully.',
        },
        data: {
          type: 'object',
          properties: {
  ...
  @ApiResponse({
    status: 400,
    description: 'Bad request - invalid file or missing post ID',
  })
  @ApiResponse({
    status: 401,
    description: 'Unauthorized - invalid or missing JWT token',
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - can only update images for own posts',
  })
  @ApiResponse({
    status: 404,
    description: 'Post not found',
  ...
  @ApiConsumes('multipart/form-data')
  @ApiBody({
    description: 'New post image file and post ID',
    schema: {
      type: 'object',
      properties: {
        file: {
          type: 'string',
          format: 'binary',
          description: 'New image file to upload for the post',
        },
        postId: {
          type: 'string',
  ...
  async updatePostImage(
    ...
    try {
      const authHeader = request.headers.authorization;
      const token = authHeader?.replace('Bearer ', '');
      const result = await this.uploadService.updatePostImage(
        request.user.id,
        postId,
        file,
        token,
      ...
      throw new HttpException(
        {
          statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
          message: 'An unexpected error occurred while updating post image.',
          error: error.message,
        },
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
